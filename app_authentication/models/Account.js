...
class Account extends Sequelize.Model {
  ...
  setPassword(password) {
    ...
    try {
      ...
      const hasRaw = crypto.pbkdf2Sync(password, salt, 12000, 64, "sha512");
      this.set("hash", Buffer.from(hasRaw).toString("hex"));
      this.set("salt",salt);
    } catch(e) {
  ...
  static passportAuthenticate() {
    return async (username, password, done) => {
      try {
        ...
        if(!account) {
          throw done(null, false, { message: "User not found" });
        }
  ...
  static async deserializeUser(account, done) {
    try {
...
Account.beforeCreate((account) => {
  account["username"] = account["username"].toLocaleLowerCase();
});
